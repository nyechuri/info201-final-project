...
```{r}
filteredMergedData1 <- mergedData1 %>% 
  group_by(states, pci) %>% 
  summarize(avg_price = mean(avg_price, na.rm = TRUE)) %>% 
  group_by(states) %>% 
  summarize(pci = first(pci), avg_price = mean(avg_price, na.rm = TRUE)) %>% 
...
```{r}
mergedData2 <- inner_join(salePrice, filteredMergedData1, by = "states")
mergedData2
```
